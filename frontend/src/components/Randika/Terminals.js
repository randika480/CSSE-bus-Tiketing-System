...
function stableSort(array, comparator, searchTerm) {
  const stabilizedThis = array
    .filter((val) => {
      if (searchTerm === "") {
        return val;
      } else if (val._id.includes(searchTerm.toLowerCase())) {
        return val;
      }
      return null;
    })
...
const headCells = [
  ...
  { id: "col3", numeric: true, disablePadding: false, label: "Name" },
...
const Inquaries = () => {
  ...
  const [TerminalModakOpen, setTerminalModakOpen] = useState(false);
    ...
    <div className="w-11/12 h-auto p-4 mt-2 m-auto pt-5 rounded-xl bg-blueSapphire bg-opacity-30">
      ...
      <div className="w-full h-auto bg-white p-3 rounded-xl">
        <div className="w-full h-16 mb-1 p-1 bg-blueSapphire bg-opacity-30 rounded-lg">
          ...
          {/* display onlu when search term not null */}
          {searchTerm && (
            <ReplayIcon
              style={{ float: "left" }}
              className="m-3"
                    ...
                    .map((row, index) => {
                      ...
                      return (
                        <TableRow hover tabIndex={-1} key={index}>
                          ...
                          >
                            <h1 className="font-bold text-md">{row.name}</h1>
                          </TableCell>
                          ...
                          >
                            <button
                              ...
                              onClick={() => {
                                setTerminalModakOpen(true);
                                setcurrentTerminal(row.distanceToOthers);
                              }}
                            >
                              View More
                            </button>
      ...
      {TerminalModakOpen && (
        <TerminalModal
          setModalVisible={setTerminalModakOpen}
          modalVisible={TerminalModakOpen}
